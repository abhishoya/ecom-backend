...
public class SecurityConfig
{
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf(AbstractHttpConfigurer::disable)
                ...
                .authorizeRequests()
                .requestMatchers("/api/user/swagger/**","/api/user/signup").permitAll()
    ...
    public class AuthFilter extends OncePerRequestFilter
    {
        ...
        protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
            ...
            tracer.currentSpan().tag("authorities", authoritiesString);
            ...
            else
            {
                ...
                response.getWriter().write(objectMapper.writeValueAsString(ExceptionResponse.builder().status(HttpStatus.UNAUTHORIZED).message("username, token or authorities missing from headers").timestamp(new Timestamp(Instant.now().toEpochMilli())).build()));
            }
        }

        @Override
        protected boolean shouldNotFilter(HttpServletRequest request) throws ServletException {
            return request.getRequestURI().matches("/api/user/swagger/(.*)") || request.getRequestURI().matches("/api/user/signup");
        }
    }
}
